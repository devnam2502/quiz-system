...
class AdminController extends Controller
{
    ...
    function deleteUser($id){
        ...
        if($isDeleted){
       Session::flash('user',"Success : User deleted.");
       return redirect("dashboard");
    ...
    function addQuiz(){
        ...
        if($admin){
            ...
            $quizData=Quiz::withCount('Records')->orderBy('records_count','desc')->take(50)->get();
            return view('add-quiz',["name"=>$admin->name,"categories"=>$categories,"totalMCQs"=>$totalMCQs, 'quizData'=>$quizData]);
        }else{
            return redirect('admin-login');
        }
    }
    ...
    if ($admin) {
        ...
        if ($quiz) {
            // Xóa tất cả các câu hỏi (MCQs) liên quan nếu cần
            Mcq::where('quiz_id', $quiz->id)->delete();

            // Xóa quiz
            $quiz->delete();

            // Nếu quiz bị xóa là quiz đang được lưu trong session thì xóa luôn session đó
            if (Session::has('quizDetails') && Session::get('quizDetails')->id == $quiz_id) {
                Session::forget('quizDetails');
            }

            return redirect()->back()->with('success', 'Quiz đã được xóa thành công.');
        } else {
            return redirect()->back()->with('error', 'Quiz không tồn tại.');
        }
    } else {
        return redirect('admin-login');
    }
